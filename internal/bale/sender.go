...
func (s *Sender) Send(ctx context.Context, msg *bridge.BridgeMessage, path string) (int64, error) {
	...
	case bridge.KindPhoto:
		if tooBig(path, MaxPhotoUploadBytes) {
			return s.client.SendDocument(ctx, s.chatID, path, msg.Caption)
		}
		return s.client.SendPhoto(ctx, s.chatID, path, msg.Caption)
	case bridge.KindVideo:
		return s.client.SendVideo(ctx, s.chatID, path, msg.Caption)
	case bridge.KindDocument:
		return s.client.SendDocument(ctx, s.chatID, path, msg.Caption)
	case bridge.KindAudio:
		return s.client.SendAudio(ctx, s.chatID, path, msg.Caption)
	case bridge.KindAnimation:
		return s.client.SendAnimation(ctx, s.chatID, path, msg.Caption)
	case bridge.KindVoice:
		if tooBig(path, VoiceAsFileLimitBytes) {
			return s.client.SendDocument(ctx, s.chatID, path, msg.Caption)
		}
		return s.client.SendVoice(ctx, s.chatID, path, msg.Caption)
	}
